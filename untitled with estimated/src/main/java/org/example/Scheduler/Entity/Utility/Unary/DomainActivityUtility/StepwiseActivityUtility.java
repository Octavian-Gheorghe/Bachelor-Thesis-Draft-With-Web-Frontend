...
public class StepwiseActivityUtility implements DomainActivityUtility
{
    ...
    public double calculateActivityForBestCaseScenario(int totalDur)
    {
        ...
        for (int dur = totalDur; dur >= a.getDurimin(); dur--)
        {
            if(dur > netSize)
                continue;
            List<ActivityPart> candidate = trySchedule(dur);
            if (candidate != null)
            {
                candidate.sort(Comparator.comparingInt(ActivityPart::getTij));
    ...
    private List<ActivityPart> trySchedule(int totalDur)
    {
    ...
    private int fillForward(List<ActivityPart> out, int startIdx, int cursor, int remaining, int idStart) {
        return fillForward(out, startIdx, cursor, remaining, idStart, a.getF().size());
    }

    private int fillForward(List<ActivityPart> out, int startIdx, int cursor, int remaining, int nextId, int stopExclusive)
    {

        for (int i = startIdx; i < stopExclusive && remaining > 0; i++) {

            TemporalInterval ti = a.getF().get(i);
            ...
            while (remaining > 0 && cursor + a.getSmini() <= ti.getBi())
            {
                ...
                out.add(new ActivityPart(nextId++, cursor, size));
                remaining -= size;
                cursor += size + a.getDmini();
            }
            cursor = Integer.MIN_VALUE;
        }
        out.sort(Comparator.comparingInt(ActivityPart::getTij));
        return remaining;
    }
